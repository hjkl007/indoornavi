...
public class SVGParser {
	...
	static SVG parse(InputSource data, SVGHandler handler)
	...
	private static Matrix parseTransform(String s) {
		...
		while (true) {
			...
			if (rparen > 0 && s.length() > rparen + 1) {
				s = TRANSFORM_SEP.matcher(s.substring(rparen + 1))
						.replaceFirst("");
			} else {
				break;
			}
	...
	private static void drawArc(Path p, float lastX, float lastY, float x,
		...
		final float R = FloatMath.sqrt((rxs * rys - rxs * y1ts - rys * x1ts)
				/ (rxs * y1ts + rys * x1ts))
		...
		float dth = angle((x1t - cxt) / rx, (y1t - cyt) / ry,
				(-x1t - cxt) / rx, (-y1t - cyt) / ry);
	...
	private static class Properties {
		...
		public Integer getColor(String name) {
			...
			if (v == null) {
				...
				try {
					return rgb(parseNum(values[0]), parseNum(values[1]),
							parseNum(values[2]));
				} catch (NumberFormatException nfe) {
					return null;
		...
		private int hex3Tohex6(int x) {
			return (x & 0xF00) << 8 | (x & 0xF00) << 12 | (x & 0xF0) << 4
					| (x & 0xF0) << 8 | (x & 0xF) << 4 | (x & 0xF);
		}
	...
	static class SVGHandler extends DefaultHandler {
		...
		RectF bounds = null;
		final RectF limits = new RectF(Float.POSITIVE_INFINITY,
				...
				if (fillString.startsWith("url(#")) {
					...
					if (shader != null) {
						...
						if (g.boundingBox && bounding_box != null) {
							...
							gradMatrix.preTranslate(bounding_box.left,
									bounding_box.top);
							gradMatrix.preScale(bounding_box.width(),
									bounding_box.height());
						}
					...
					if (color != null) {
						...
						Log.w(TAG, "Unrecognized fill color, using black: "
								+ fillString);
						doColor(atts, Color.BLACK, true, fillPaint);
						return true;
					}
			...
			if (strokeString != null) {
				if (strokeString.equalsIgnoreCase("none")) {
					...
					if (color != null) {
						...
						Log.w(TAG, "Unrecognized stroke color, using none: "
								+ strokeString);
						strokePaint.setColor(Color.TRANSPARENT);
						return false;
					}
		...
		private void doColor(Properties atts, Integer color, boolean fillMode,
			...
			if (opacityAttr == null) {
				opacityAttr = atts.getFloat(fillMode ? "fill-opacity"
						: "stroke-opacity");
			}
		...
		public void startElement(String namespaceURI, String localName,
			...
			if (localName.equals("svg")) {
				...
				// No viewbox
				if (canvas == null) {
					int width = (int) FloatMath
							.ceil(getFloatAttr("width", atts));
					int height = (int) FloatMath.ceil(getFloatAttr("height",
							atts));
				...
				if ("none".equals(getStringAttr("display", atts))
						|| "none".equals(props.getString("display"))) {
					if (!hidden) {
						hidden = true;
						hiddenLevel = 1;
						// Util.debug("Hidden up: " + hiddenLevel);
				...
				LayerAttributes newLayerAttr = new LayerAttributes(
						curLayerAttr.opacity * opacity);
				...
				if (id != null) {
					rectID = id;
					Float textLen = textPaint.measureText(id);
					if(width >= height){
						Float extraSpace = width - textLen;
						canvas.drawText(id, x + (extraSpace / 2), rect.centerY(),
								textPaint);
					}else{
						Float extraSpace = height - textLen;
						canvas.save();
						canvas.rotate(-90, rect.centerX(), rect.centerY());
						canvas.drawText(id, rect.centerX()-(height/2)+extraSpace, rect.centerY(), textPaint);
						canvas.restore();
					}
				...
				if (doStroke(props)) {
					...
					&& (localName.equals("circle") || localName
							.equals("ellipse"))) {
			...
			if (localName.equals("svg")) {
				...
				if (gradient.id != null) {
					...
					if (localName.equals("linearGradient")) {
						...
						gradient.shader = new RadialGradient(gradient.x,
