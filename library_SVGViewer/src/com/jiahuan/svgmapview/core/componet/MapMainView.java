...
public class MapMainView extends SurfaceView implements Callback
{
    ...
    private float minZoomValue = 1.0f; // 默认的最小缩放
    ...
    public void loadMap(final String svgString, final Context context)
    {
        ...
        new Thread()
        {
            ...
            {
                super.run();
                Picture picture = new SVGBuilder().readFromString(svgString).build(context).getPicture();
                if (picture != null)
                {
